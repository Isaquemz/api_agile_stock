...
# Entrada
from movimentos.models import Entrada
from movimentos.serializers import EntradaSerializer
    ...
    def put(self, request, pk=None):
        ...
        if not isinstance(has_acesso, JsonResponse) and has_acesso:
            try:
                ...
                return Response(status=status.HTTP_404_NOT_FOUND)
            serializer = EntradaSerializer(entrada, data=request.data, partial=True)
        else:
            return Response({'detail': 'Não autorizado.'}, status=status.HTTP_403_FORBIDDEN)
