...
# Entrada Produto
from movimentos.models import EntradaProduto
from movimentos.serializers import EntradaProdutoSerializer
...
# Views
class EntradaProdutoAPIView(APIView):

    def __init__(self):

        # Funcionalidade da Classe
        self.co_funcionalidade = 9
        super().__init__()
    ...
    def get(self, request, pk=None):
        ...
        if self.has_permission(request, co_acao):
            if pk is None:
                ...
                serializer = EntradaProdutoSerializer(entrada)
    ...
    def put(self, request, pk=None):
        ...
        if self.has_permission(request, co_acao):
            try:
                ...
                return Response(status=status.HTTP_404_NOT_FOUND)
            serializer = EntradaProdutoSerializer(entrada, data=request.data, partial=True)
        else:
            return Response({'detail': 'Não autorizado.'}, status=status.HTTP_403_FORBIDDEN)
