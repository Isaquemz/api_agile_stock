...
# Saida Produto
from movimentos.models import SaidaProduto
from movimentos.serializers import SaidaProdutoSerializer
...
# Views
class SaidaProdutoAPIView(APIView):

    def __init__(self):

        # Funcionalidade da Classe
        self.co_funcionalidade = 10
        super().__init__()
    ...
    def get(self, request, pk=None):
        ...
        if not isinstance(has_acesso, JsonResponse) and has_acesso:
            if pk is None:
                ...
                serializer = SaidaProdutoSerializer(saida)
    ...
    def put(self, request, pk=None):
        ...
        if not isinstance(has_acesso, JsonResponse) and has_acesso:
            try:
                ...
                return Response(status=status.HTTP_404_NOT_FOUND)
            serializer = SaidaProdutoSerializer(saida, data=request.data, partial=True)
        else:
            return Response({'detail': 'Não autorizado.'}, status=status.HTTP_403_FORBIDDEN)
