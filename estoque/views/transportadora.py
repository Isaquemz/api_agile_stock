...
# Transportadora
from estoque.models import Transportadora
from estoque.serializers import TransportadoraSerializer
...
# Views
class TransportadoraAPIView(APIView):

    def __init__(self):

        # Funcionalidade da Classe
        self.co_funcionalidade = 7
        super().__init__()
    ...
    def put(self, request, pk=None):
        ...
        if not isinstance(has_acesso, JsonResponse) and has_acesso:
            try:
                ...
                return Response(status=status.HTTP_404_NOT_FOUND)
            serializer = TransportadoraSerializer(transportadora, data=request.data, partial=True)
        else:
            return Response({'detail': 'Não autorizado.'}, status=status.HTTP_403_FORBIDDEN)
